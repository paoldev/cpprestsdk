...
static void parse_reason_phrase(HINTERNET request_handle, utility::string_t& phrase)
{
    ...
    trim_nulls(buffer);
    phrase = utility::conversions::to_string_t(buffer);
}
...
static void parse_winhttp_headers(HINTERNET request_handle, utf16char* headersStr, test_response* p_response)
{
    ...
    while (line != nullptr)
    {
        const utility::string_t header_line = utility::conversions::to_string_t(line);
...
public:
    ...
    unsigned long open()
    {
        ...
        m_hConnection =
            WinHttpConnect(m_hSession, utility::conversions::to_utf16string(u.host()).c_str(), (INTERNET_PORT)port, 0);
    ...
    unsigned long request(const utility::string_t& method,
                          ...
                          size_t data_length)
    {
        HINTERNET request_handle = WinHttpOpenRequest(m_hConnection,
        ...
        if (!headers.empty())
        {
            utf16string flattened_headers = utility::conversions::to_utf16string(flatten_http_headers(headers));
    ...
    completion_callback(HINTERNET hRequestHandle, DWORD_PTR context, DWORD statusCode, void* statusInfo, DWORD)
    {
        ...
        if (p_response != nullptr)
        {
            ...
            else if (statusCode == WINHTTP_CALLBACK_STATUS_HEADERS_AVAILABLE)
            {
                ...
                header_raw_buffer.resize(headers_length, '\0');
